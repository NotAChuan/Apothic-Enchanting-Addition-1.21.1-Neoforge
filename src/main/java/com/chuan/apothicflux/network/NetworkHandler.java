...
public class NetworkHandler {
    ...
    public static void handleFluxClue(final FluxCluePayload payload, final IPayloadContext context) {
        context.enqueueWork(() -> {
            ...
            if (player != null && player.containerMenu instanceof com.chuan.apothicflux.menu.FluxEnchantingMenu menu) {
        ...
        context.enqueueWork(() -> {
            if (context.player() instanceof ServerPlayer player) {
                ...
                if (player.distanceToSqr(payload.pos().getX() + 0.5, payload.pos().getY() + 0.5, payload.pos().getZ() + 0.5) <= 256.0) {
                    ...
                    if (player.containerMenu instanceof com.chuan.apothicflux.menu.FluxEnchantingMenu menu) {
